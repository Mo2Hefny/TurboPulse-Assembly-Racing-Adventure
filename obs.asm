  ...
  EXTRN ACTUAL_ENTITIES_COUNT:WORD
  EXTRN ENTITIES_COUNT:WORD
  EXTRN ENTITIES_TYPE:WORD
  EXTRN ENTITIES_X:WORD
  EXTRN ENTITIES_Y:WORD
  ; CARS.asm
...
RANDOM_SPAWN_ENTITY proc near
  ...
  jnz EXIT_RANDOM_SPAWN
  mov RANDOM_SPAWN_TIME, RANDOM_SPAWN
  mov BX, ACTUAL_ENTITIES_COUNT
  cmp BX, ENTITIES_COUNT
  jz EXIT_RANDOM_SPAWN
  mov AX, 10
  cmp ENTITIES_TYPE[BX], AX
  jl EXIT_RANDOM_SPAWN
  sub ENTITIES_TYPE[BX], AX
  mov AX, 2
  add [ACTUAL_ENTITIES_COUNT], AX
  EXIT_RANDOM_SPAWN:
  ret
...
CHECK_ENTITY_COLLISION proc near
  ...
  cmp AX, 10
  jge EXIT_CHECK_ENTITY_COLLISION
  push BX
  mov BX, AX
